...
#        Exec rubric 40/30/20/10 (badge "Delivers for the People"); judicial rubric
#        30/25/25/20 (badge "Fair-Bench Distinction"); 80 = Stability Standard, fixed.
#        Components are set by review in the Q Console (verified before export); this
#        engine only applies the locked weights. Runs even when the LegiScan pull is
#        skipped, so exec/judicial move on their own cadence (curation / retention).
#   v6.5 enacted-count fix (corrects DISPLAY and SCORE): the raw sponsored list repeated the
#        same bill_id across companion/cross-chamber rows, so prime-sponsored "enacted" was
#        badly inflated (e.g. Sirota 117 -> real 18, Kyle Brown 122 -> real 7). Now de-dupes
#        primary bills by bill_id before counting. Fixes the number shown on profiles AND the
#        objective-sponsorship input to the score (members under 10 real enactments were over-
#        credited). SCORE_VERSION bumped to v6.5 -> forces one full re-score so every member is
#        recomputed with honest counts. No rubric/weights changed; only the input was corrected.
...
SCORE_VERSION = "v7.0"        # bump to force a full re-score on the next run
...
def do_state(abbr, ds):
    ...
    for pid, a in items:
        ...
        # v6.5: de-duplicate prime-sponsored bills by bill_id before counting.
        # Colorado lists a member as primary on companion/cross-chamber rows, so the raw
        # sponsored list repeats the same bill_id many times - which inflated "enacted"
        # (e.g. 117 shown vs 18 real). De-duping gives the true count of bills LED.
        _seen_bill = set(); primary = []
        for s in a["sponsored"]:
            if s["primary"] and s["bill_id"] not in _seen_bill:
                _seen_bill.add(s["bill_id"]); primary.append(s)
        co_count = len({s["bill_id"] for s in a["sponsored"] if not s["primary"]} - _seen_bill)
...
EXEC_W = {"pillar_alignment":0.40, "citizen_impact":0.30, "implementation_delivery":0.20, "transparency":0.10}
JUD_W  = {"integrity_impartiality":0.30, "access_citizen_impact":0.25, "competence_consistency":0.25, "transparency_accountability":0.20}

def _wscore(comp, W):
    return round(sum(float((comp or {}).get(k, 0)) * w for k, w in W.items()))
...
def _jrow(j):
    s = _wscore(j.get("components", {}), JUD_W)
    return {"name": j.get("name",""), "evaluation_year": j.get("evaluation_year",""),
            "next_retention": j.get("next_retention",""),
            "ojpe_recommendation": j.get("ojpe_recommendation",""), "score": s,
            "label": ("Fair-Bench Distinction" if s >= 80 else ""),
            "components": j.get("components", {}),
            "narrative_note": j.get("narrative_note", j.get("note","")),
            "source": j.get("source",""), "verified": bool(j.get("verified", False))}

def merge_curated(abbr):
    """Attach curated executive/judicial scores to data/<abbr>.json.
    Tolerant of the Q Console's export shape: executive offices may be a dict
    keyed by office name OR a list; judicial justices may sit under a "courts"
    list OR under a court-named key (e.g. "Colorado Supreme Court"). The engine
    RE-COMPUTES each score from the verified components with the locked weights,
    so the Console preview and the site can never disagree. Runs regardless of
    the LegiScan skip. Missing files are a silent no-op."""
    state_file = os.path.join(OUTDIR, abbr + ".json")
    ...
    ex = _load_json(os.path.join(OUTDIR, "executive.json"))
    if ex and ex.get("state") == abbr:
        raw = ex.get("offices", [])
        items = raw.items() if isinstance(raw, dict) else [((o.get("office") or ""), o) for o in raw]
        rows = []
        for name, o in items:
            status = o.get("status", "in_development")
            comps = o.get("components", {})
            s = _wscore(comps, EXEC_W) if status == "scored" else None
            rows.append({"office": name, "holder": o.get("holder",""), "status": status,
                         "score": s, "label": ("Delivers for the People" if (s is not None and s >= 80) else ""),
                         "components": comps, "entries": o.get("entries", o.get("evidence", [])),
                         "why": o.get("why", o.get("note",""))})
        data["executive"] = rows; changed = True

    jd = _load_json(os.path.join(OUTDIR, "judicial.json"))
    if jd and jd.get("state") == abbr:
        courts = []
        if isinstance(jd.get("courts"), list):
            for c in jd["courts"]:
                courts.append({"court": c.get("court",""), "justices": [_jrow(x) for x in c.get("justices", [])]})
        else:
            for k, v in jd.items():
                if isinstance(v, list):
                    courts.append({"court": k, "justices": [_jrow(x) for x in v]})
        data["judicial"] = courts; changed = True

    if changed:
        data["scoring_version"] = SCORE_VERSION
...
def main():
    ...
    # v7.0: attach curated executive/judicial (runs even for skipped states)
    for abbr in sorted(chosen.keys()):
