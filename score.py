...
#   v3  HARSH RECALIBRATION
#       - Default model -> Claude Sonnet (sharper, consistent grading)
#       - Rewritten standard: 80+ is RARE and earned; most legislators
#         land 40-60; co-sponsorship counts for little; judged mainly on
#         bills they PRIME-SPONSORED and actually got enacted
#       - Demands real differentiation (no more stock/clustered scores)
#       - Attendance rescaled: showing up is expected, not rewarded
#         (the 80-100% band is spread out; below 90% drops fast)
#       - FIXED chamber detection (Senator/Representative was mislabeled)
#       - Cleaned district numbers ("SD-029" -> "29")
#       - Cache auto-invalidates when scoring logic changes (SCORE_VERSION)
...
MODEL         = os.environ.get("MODEL", "claude-sonnet-4-6")   # sharper grading; set to a Haiku string to cut cost
LIMIT         = int(os.environ.get("LIMIT", "0"))
OUTDIR        = "data"
SCORE_VERSION = "v3"          # bump this to force a full re-score on the next run
...
def stamp_now():
    """Current time in Mountain Time (handles MST/MDT automatically)."""
    try:
...
def attendance_score(raw):
    """Showing up is expected. Spread the 80-100% band; below 90% drops fast."""
...
def score_person(name, role, state_name, primary_bills, co_count):
    ...
    prompt = (
        ...
        "GRADING STANDARD - be harsh and discriminating. The scale is calibrated like this:\n"
        "  80-100  RARE. Personally LED (prime-sponsored) multiple substantial bills that BECAME LAW and "
        "directly advance the pillars. Most legislators never reach this.\n"
        "  60-79   Strong. Prime-sponsored several meaningful pillar-aligned bills, some enacted.\n"
        "  40-59   AVERAGE - where most legislators belong. A few prime sponsorships, modest impact, "
        "or mostly co-sponsorships.\n"
        "  20-39   Thin. Little prime sponsorship; mostly symbolic, narrow, or unrelated bills.\n"
        "  0-19    Nothing that advances the pillars.\n\n"
        "Co-sponsoring popular bills is EASY and counts for very little. Judge mainly what they "
        "PRIME-SPONSORED and what actually BECAME LAW. Two legislators with different records MUST get "
        "different scores - do not output generic or clustered numbers.\n\n"
        "Legislator: " + name + " (" + role + ", " + state_name + ")\n"
        "Bills they PRIME-SPONSORED / led (" + str(len(primary_bills)) + "):\n" + plines + "\n"
        ...
        "  sponsorship  - leadership: how many weighty pillar-aligned bills they prime-sponsored and got enacted\n\n"
        'Return ONLY compact JSON, no prose: {"pillar":N,"impact":N,"sponsorship":N,"why":"one specific sentence about THIS legislator\'s record"}')
    data = anthropic_call({"model": MODEL, "max_tokens": 320,
...
def load_cache(path):
    try:
        return {str(l["id"]): l for l in json.load(open(path)).get("legislators", [])}
    except Exception:
        return {}
...
def do_state(abbr, ds):
    ...
    for pid, a in items:
        ...
        out.append({"id": str(pid), "name": nm, "role": role, "party": p.get("party",""),
                    "district": clean_district(p), "score": total,
    ...
    data = {"state": abbr, "state_name": name, "session": ds["session_id"], "session_name": ds.get("session_name",""),
            "updated": stamp_now(), "count": len(out), "legislators": out, "recent_bills": recent}
    with open(os.path.join(OUTDIR, abbr + ".json"), "w") as f:
        json.dump(data, f, indent=1)
    log("  wrote %s.json  (%d legislators: %d scored, %d cached) - %d champions (80+)" %
        (abbr, len(out), scored, reused, champs))
